...
Silicon spin qubits — single electrons trapped and controlled in "quantum dots"
made from ordinary CMOS transistor gates — are one of the leading candidates for
building a large-scale quantum computer, because they can in principle be
manufactured on the same industrial silicon lines that already make every
processor in existence. imec, one of the world's leading semiconductor research
institutes, has fabricated and published a real 300 mm silicon quantum-dot chip
along these lines. Before proposing any *new* device architecture, the right first
step is to prove a simulation tool can correctly reproduce a device that has
already been built and measured. That is what this repository does, in two parts:

- **Part 1 (complete):** build a physics simulation of imec's actual published
  quantum-dot chip from scratch, and check it against imec's own measurements and
  simulations. This is the validation work — proof the tool and the modeling
  approach are trustworthy.
- **Part 2 (in progress):** use that validated tool to simulate a new,
  not-yet-built device concept — a quantum-dot cell built on **CFET**, the
  semiconductor industry's next-generation transistor architecture — which is the
  actual research contribution this project is working toward.

**Headline result of Part 1:** the simulation independently reproduces five
different physical quantities from imec's own published work — a gate-to-dot
capacitance, a turn-on voltage, an energy-level spacing, an electric field, and a
gate material property — each landing inside imec's own published range or
matching exactly, plus a <0.1% match to an independent vendor benchmark. Full
numbers in §3.
...
| **Validated against** | A vendor reference device, published literature benchmarks, and imec's own measured + simulated quantum-dot array |
...
plunger layer, and a barrier layer — think of them as three sets of "fingers"
crossing over each other to box in a single electron). This repository builds and
simulates a 3×3 interior sub-section of that array — real gate widths, real oxide
thicknesses, taken directly from imec's publications.
...
trapped — three possible dot locations along the active gate row, the center one
used for every result below.*
...
Pipeline: build the 3D gate geometry → solve for the electric potential →
solve for the confined electron's quantum states in the region where the dot
forms → sweep gate voltages to extract how the dot responds → compare every
extracted quantity against imec's publications. Full detail:
...
the fair, apples-to-apples comparison — it matches closely on every count. Where
it diverges from imec's *measured* chip, the divergence has a specific, physical,
...
and/or a tunable electrostatic screen — controlled independently through a gap
that already exists between the two transistors in the standard CFET process.
...
opposite regime: **two independent systems** — one is the qubit, the other is a
supporting/sensing tier — that are electrostatically coupled but functionally
separate, each independently controllable, in a manufacturing process the
semiconductor industry is already building for ordinary logic chips. A
semiconductor supporting tier is also continuously tunable by adjusting its
carrier density — something a fixed metal shield (the conventional approach to
electrostatic screening) cannot do.
...
capacitance and gate sensitivity — carries over directly; only the device
geometry needs to change. Results will be added to this README as they are
produced.
